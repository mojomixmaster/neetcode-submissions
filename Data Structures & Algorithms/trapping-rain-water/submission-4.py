class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        # by noticing a hidden recurrence relation: maxleft[i] is just maxleft[i-1] extended by one element
        ...
        for i in range(1, len(height)-1):
            ...
            if height[i] < height[highest_left_edge_index] and height[i] < highest_right_edges[i]: # for col i to hold water above it, it must have adjacent edges that are BOTH higher
                total_water_stored += min(height[highest_left_edge_index], highest_right_edges[i]) - height[i]
