class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        # if r = l+1 then we are down to one number only, send it to final evaluation statement.
            ...
            if target == midpoint_value: return midpoint_idx
                ...
                l = midpoint_idx+1 # midpoint_val has just been evaluated and found not to be == target, discard from search
            else: # target is in 1st half of arr ie arr[:midpoint_idx]
                r = midpoint_idx-1 # discard midpoint_val from search (same as above)
