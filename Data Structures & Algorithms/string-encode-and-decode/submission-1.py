class Solution:
    ...
    def decode(self, s: str) -> List[str]:
        ...
        marker_search_start_index = 0
        while current_idx_pointer < len(s):
            if s[current_idx_pointer] == '#':
                str_length = int(s[marker_search_start_index:current_idx_pointer]) # every character from end of last str index to this hash comprises the length of the following string
                decoded_string.append(s[current_idx_pointer+1: current_idx_pointer+1+str_length])
                
                current_idx_pointer += (str_length+1)
                marker_search_start_index = current_idx_pointer
