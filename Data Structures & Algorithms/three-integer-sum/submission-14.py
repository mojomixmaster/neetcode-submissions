class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        # IMPORTANT: we need a dedupe-during-construction mechanism!!
        triplets = []
        ...
        # all possible solutions with a fixed pointer of nums[0, ..., i-1] have been exhausted/fully considered
            ...
            while j < k:
                ...
                if three_sum == 0:
                    ...
                    k -= 1
                    while j < k and nums[j-1] == nums[j]: # ensure pointer is still valid AND our finger is not on a duplicate number
                        j += 1 
                    while k > j and nums[k+1] == nums[k]: # same as above
                        k -= 1
