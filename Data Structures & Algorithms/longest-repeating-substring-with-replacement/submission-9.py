class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        while r < len(s): # algorithm iterates until right pointer is at end of array
            ...
            r += 1
            length_of_longest_substring = max(length_of_longest_substring, r-l) # the char we've just moved onto might make the window invalid - don't count it in
            # The window slides without growing, so even though window is now over diff chars, (r - l) stays the same and you never record a false maximum. 
            # The answer only grows when the window genuinely expands past a previous best.
